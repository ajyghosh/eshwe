...
import { checkCheckout, clearPendingCheckout, openCheckout, pendingCheckout, type CheckoutResponse } from "@/lib/checkout";
...
export function OrderStatusPage({ mobile = false }: { mobile?: boolean }) {
  ...
  useEffect(()=>{
    ...
    const refresh=async()=>{
      try{const response=await checkCheckout(id);if(!active)return;setState({uid,result:response});setError(null);
        ...
        if(response.order && (response.order.paymentCaptured||["released","exception"].includes(response.order.reservationState||"")))clearPendingCheckout(uid);
  ...
  async function cancel(){if(!result?.internalOrderId)return;setBusy(true);try{const response=await checkCheckout(result.internalOrderId,true);setState({uid:user!.uid,result:response});if(response.order?.reservationState==="released")clearPendingCheckout(user!.uid);}catch(e){setError(e instanceof Error?e.message:"Unable to cancel.");}finally{setBusy(false);}}
  async function resume(){if(!result)return;setBusy(true);try{const fresh=await checkCheckout(result.internalOrderId);await openCheckout(fresh,()=>{setBusy(false);setTick(t=>t+1);},()=>{setBusy(false);setTick(t=>t+1);},message=>{setBusy(false);setError(message);});}catch(e){setBusy(false);setError(e instanceof Error?e.message:"Unable to resume payment.");}}
  return <main className="min-h-screen bg-[#fbf4e8] px-5 py-12 text-[#2f342d]"><section className="mx-auto max-w-2xl rounded-3xl border border-[#dfd2bd] bg-white p-6 sm:p-10">
    ...
    {loading?<p className="mt-4">Checking your account…</p>:!user?<><p className="mt-4">Sign in with the account used at checkout to view this order.</p><button onClick={()=>void signIn().catch(e=>setError(e.message))} className="mt-5 rounded-xl bg-[#5e684f] px-5 py-3 text-white">Sign in</button></>:<>
      ...
      {!result&&!error?<p className="mt-5" role="status">Checking payment with the server…</p>:null}
      {order?<><p className="mt-5 break-all text-sm">Order #{order.id||result?.internalOrderId}</p><p className="mt-4 text-xl font-semibold">{paymentLabel(order)}</p><p className="mt-2">{fulfilmentLabel(order)}</p>
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          {result?.canPay?<button disabled={busy} onClick={()=>void resume()} className="rounded-xl bg-[#5e684f] px-5 py-3 text-white">Resume payment</button>:null}
          {order.reservationState==="held"&&!order.paymentCaptured?<button disabled={busy} onClick={()=>void cancel()} className="rounded-xl border px-5 py-3">Cancel reservation</button>:null}
