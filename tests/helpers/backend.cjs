...
function loadBackend(db, overrides={}){
 ...
 vm.runInNewContext(fs.readFileSync(filename,'utf8')+'\nmodule.exports.testOnly={normalizeCustomer,verifyStoredCustomerCode,verifyPaymentForOrder,hashVerificationCode,hashValue,buildOrderConfirmationPayload,handleWebhookEvent};',{require:id=>id in mocks?mocks[id]:localRequire(id),module,exports:module.exports,process:{env:overrides.env||{}},Buffer,console,URL,Date,Error,fetch:overrides.fetch||(()=>{throw Error('Unexpected external request');}),AbortSignal},{filename});
 // Fixture routes keep logical names while invoking the actual US exports.
 for(const [name,handler] of Object.entries(module.exports))if(name.endsWith('Us'))module.exports[name.slice(0,-2)]=handler;
